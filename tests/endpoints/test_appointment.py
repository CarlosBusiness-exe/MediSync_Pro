...
def created_doctor(client):
    ...
    response = client.post(f"{settings.API_V1_STR}/doctors/", json=doctor)
...
def created_patient(client):
    ...
    response = client.post(f"{settings.API_V1_STR}/patients/", json=patient)
    return response.json()

def test_create_ap_success(client, session, created_doctor, created_patient):
    appointment = {
        "ap_date": "2026-03-30",
        "ap_time": "13:00:00",
    ...
    response = client.post(f"{settings.API_V1_STR}/appointments/", json=appointment)
    data = response.json()
...
def test_create_ap_missing(client, created_patient):
    payload = {
        "ap_time": "13:00:00",
        "patient_id": created_patient["id"],
        "status": "Pending"
    ...
    response = client.post(f"{settings.API_V1_STR}/appointments/", json=payload)
    assert response.status_code == 422

def test_create_ap_invalid(client, created_patient):
    appointment = {
        "ap_date":"2026,03,30",
        "ap_time":"13,00,00",
        "doctor_id":"string",
...
def test_create_ap_same_time(client, created_doctor, created_patient):
    appointment = {
        "ap_date":"2026-03-30",
        "ap_time":"13:00:00",
        "doctor_id": created_doctor["id"],
    ...
    st_response = client.post(f"{settings.API_V1_STR}/appointments/", json=same_time_ap)
...
def test_get_invalid_id(client):
    ...
    response = client.get(f"{settings.API_V1_STR}/appointments/{invalid_id}")
